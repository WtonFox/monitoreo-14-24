...
import { CalendarDays, TrendingUp, Calendar, Clock, Building2, Grid3X3, List, RotateCcw } from 'lucide-react'
...
const RegistroDiarioBoard: React.FC = () => {
  ...
  const metrics: ComputedMetrics = useMemo(() => {
    ...
    const totalPeriodo = Array.from(dailyCounts.values()).reduce((s, c) => s + c, 0);
    const diasConDatos = dailyCounts.size;
    const promedioDiarioRange = diffDays >= 0 ? Math.round((totalPeriodo / (diffDays + 1)) * 10) / 10 : 0;
    ...
    const timelineWithMA = timeline.length >= 7
    ...
    description="Monitoreo diario de fichas registradas: KPIs, timeline de 30 días y ranking de centros.">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">Total en el período</p>
            <h3 className="text-2xl font-bold text-gray-800">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-wrap items-center gap-3">
        <IndicadoresFilterBar showYear showProvince showMunicipio showSex={false} noContainer />
        ...
        <div className="ml-auto flex items-center gap-2">
          <BoardInfo
            ...
            sections={[
              ...
              { heading: 'KPIs', content: '• Fichas Hoy: registros del día actual\n• Total en el período: suma de fichas en el rango de fechas seleccionado\n• Días con datos: cantidad de días con al menos un registro\n• Rango de días: días cubiertos por el período seleccionado\n• Promedio diario: total / días del período' },
              { heading: 'Timeline', content: 'El gráfico principal muestra barras diarias del período seleccionado con una línea de media móvil de 7 días (promedio semanal) para suavizar la tendencia.' },
              ...
              { heading: 'Ranking de Centros', content: 'Los centros se ordenan por cantidad total de fichas registradas.' },
              { heading: 'Filtros', content: 'Usá los filtros globales para segmentar por provincia, municipio y año. El selector de fechas permite analizar cualquier período.' },
            ]}
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mb-2">Valores acumulados del período seleccionado</p>
          <div className="h-44 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.dayOfWeek}>
                ...
                <YAxis tickFormatter={formatNumber} />
                <Tooltip formatter={(v: unknown) => formatNumber(Number(v))} />
                <Bar dataKey="value" fill="#0891b2" radius={[4, 4, 0, 0]} name="Fichas" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-right py-2 px-3 font-medium text-gray-500">Fichas</th>
                </tr>
              </thead>
              <tbody>
                {metrics.centrosHoy.slice(0, 10).map((row, idx) => (
                  <tr key={row.centro} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3 text-gray-800 font-medium">{row.centro}</td>
                    <td className="py-2 px-3 text-gray-500">{row.provincia || '—'}</td>
                    <td className="py-2 px-3 text-right text-gray-800">{formatNumber(row.count)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
